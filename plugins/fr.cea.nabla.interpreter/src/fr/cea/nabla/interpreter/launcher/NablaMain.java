...
public final class NablaMain {
    ...
    public static void main(String[] args) throws IOException {
        ...
        if (file == null) {
            // @formatter:off
            source = Source.newBuilder(NABLA, new InputStreamReader(System.in), "<stdin>").build();
            // @formatter:on
        } else {
            source = Source.newBuilder(NABLA, new File(file)).build();
        }
    ...
    private static int executeSource(Source source, InputStream in, PrintStream out, Map<String, String> options) {
        ...
        try {
            context = Context.newBuilder(NABLA).in(in).out(out).options(options).build();
        ...
        try {
            ...
            if (context.getBindings(NABLA).getMember("main") == null) {
