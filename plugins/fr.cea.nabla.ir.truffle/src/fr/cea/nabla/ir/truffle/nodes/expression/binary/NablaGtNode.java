...
@NodeInfo(shortName = ">")
public abstract class NablaGtNode extends NablaBinaryExpressionNode {
	...
	@Specialization
	protected NV0Bool gt(NV0Int left, NV0Int right) {
		return new NV0Bool(left.getData() > right.getData());
	}
	
	@Specialization
	protected NV0Bool gt(NV0Int left, NV0Real right) {
		return new NV0Bool(left.getData() > right.getData());
	}
	
	
	@Specialization
	protected NV0Bool gt(NV0Real left, NV0Int right) {
		return new NV0Bool(left.getData() > right.getData());
	}
	
	@Specialization
	protected NV0Bool gt(NV0Real left, NV0Real right) {
		return new NV0Bool(left.getData() > right.getData());
	}
