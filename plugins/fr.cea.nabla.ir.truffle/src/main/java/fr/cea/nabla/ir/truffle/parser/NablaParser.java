		...
		if (postProcessingInfo != null) {
			...
			if (instrument != null) {
				...
				final int offset = Integer.parseInt(irModule.getMainTimeLoop().getAnnotations().stream()
						.filter(a -> a.getSource().equals("nabla-origin")).findFirst().map(a -> a.getDetails().stream()
								.filter(e -> e.getKey().equals("offset")).findFirst().map(e -> e.getValue()).orElse(""))
						.orElse(""));

				instrument.configure(offset, irModule.getName(), iterationVariable, timeVariable, periodVariable,
